...
static char humidity_data[10];
static int humidity_data_len = 0;
static char temperature_data[10];
static int temperature_data_len = 0;
...
#define INITIAL_DATA "n/a"
#define STATIC_DATA "ESP32 Sensor node with DHT-22"

/*
 * The resource handler
 */
static void
hnd_description_get(coap_context_t *ctx, coap_resource_t *resource,
...
static void
hnd_humidity_get(coap_context_t *ctx, coap_resource_t *resource,
                  coap_session_t *session,
                  coap_pdu_t *request, coap_binary_t *token,
                  coap_string_t *query, coap_pdu_t *response)
{
    int ret = readDHT();
    errorHandler(ret);
    snprintf(humidity_data, 8, "\"%.1f%%\"\n", getHumidity());
    humidity_data_len = strlen(humidity_data);
    coap_add_data_blocked_response(resource, session, request, response, token,
                                   COAP_MEDIATYPE_TEXT_PLAIN, 0,
                                   (size_t)humidity_data_len,
                                   (const u_char *)humidity_data);
}

static void
hnd_temperature_get(coap_context_t *ctx, coap_resource_t *resource,
                  coap_session_t *session,
                  coap_pdu_t *request, coap_binary_t *token,
                  coap_string_t *query, coap_pdu_t *response)
{
    int ret = readDHT();
    errorHandler(ret);
    snprintf(temperature_data, 8, "\"%.1fC\"\n", getTemperature());
...
static void
hnd_description_put(coap_context_t *ctx,
...
static void
hnd_description_delete(coap_context_t *ctx,
                     coap_resource_t *resource,
                     coap_session_t *session,
                     coap_pdu_t *request,
                     coap_binary_t *token,
...
static void coap_example_server(void *p)
{
    ...
    coap_resource_t *resource_des = NULL;
    coap_resource_t *resource_hum = NULL;
    coap_resource_t *resource_tmp = NULL;

    snprintf(espressif_data, sizeof(espressif_data), STATIC_DATA);
    snprintf(humidity_data, sizeof(espressif_data), INITIAL_DATA);
    snprintf(temperature_data, sizeof(espressif_data), INITIAL_DATA);
    espressif_data_len = strlen(espressif_data);
    humidity_data_len = strlen(humidity_data);
    temperature_data_len = strlen(temperature_data);
	...
	setDHTgpio( 26 );
	printf( "Starting DHT via CoAP Task\n\n");
...
/*        unsigned wait_ms;*/
        ...
        resource_des = coap_resource_init(coap_make_str_const("description"), 0);
        if (!resource_des) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        resource_hum = coap_resource_init(coap_make_str_const("humidity"), 0);
        if (!resource_hum) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        resource_tmp = coap_resource_init(coap_make_str_const("temperature"), 0);
        if (!resource_tmp) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_des, COAP_REQUEST_GET, hnd_description_get);
        coap_register_handler(resource_hum, COAP_REQUEST_GET, hnd_humidity_get);
        coap_register_handler(resource_tmp, COAP_REQUEST_GET, hnd_temperature_get);
        coap_register_handler(resource_des, COAP_REQUEST_PUT, hnd_description_put);
        coap_register_handler(resource_des, COAP_REQUEST_DELETE, hnd_description_delete);
        /* We possibly want to Observe the GETs */
        coap_resource_set_get_observable(resource_des, 1);
        coap_resource_set_get_observable(resource_hum, 1);
        coap_resource_set_get_observable(resource_tmp, 1);
        coap_add_resource(ctx, resource_des);
        coap_add_resource(ctx, resource_hum);
        coap_add_resource(ctx, resource_tmp);
